...
const UpdateBooks = () => {
    ...
    const getBookDetails = async () => {
        let result = await axios.get(`http://localhost:5010/book/${params.id}`);
        setBName(result.BName);
    ...
    const updateBook = async (e) => {
        ...
        const result = await axios.put(`http://localhost:5010/book/${params.id}`,
            formData,
            {
    ...
    return (
        <table align='center'>
            <div className='product'>
                ...
                <button type='button' onClick={updateBook} className='btn' >Update Book</button>
            </div>
        </table>
    )
}
